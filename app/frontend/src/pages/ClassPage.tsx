...
export default function ClassPage() {
  ...
  // Sort teachers by current_students descending (most students first)
  const sortedTeachers = useMemo(() => {
    return [...teachers].sort((a, b) => b.current_students - a.current_students);
  }, [teachers]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {sortedTeachers.length === 0 && (
          <div className="col-span-full text-center py-12">
            <p className="text-gray-400">선생님이 없습니다.</p>
          </div>
        )}
        {sortedTeachers.map((teacher) => {
          ...
          return (
            <Card key={teacher.id} className={`bg-gray-900 border-gray-800 hover:border-gray-600 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${isFull ? 'opacity-70' : ''}`}>
